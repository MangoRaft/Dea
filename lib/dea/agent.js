 ...
 */
const VERSION = 0.99
const DEFAULT_APP_MEM = 512//512MB
const DEFAULT_APP_DISK = 256//256MB
const DEFAULT_APP_NUM_FDS = 1024

// Max limits for DEA
const DEFAULT_MAX_CLIENTS = 1024

const MONITOR_INTERVAL = 2000// 2 secs
const MAX_USAGE_SAMPLES = (1 * 60000) / MONITOR_INTERVAL// 1 minutes this. 5 sec interval
const CRASHES_REAPER_INTERVAL = 30000// 30 secs
const CRASHES_REAPER_TIMEOUT = 3600000// delete crashes older than 1 hour

// CPU Thresholds
const BEGIN_RENICE_CPU_THRESHOLD = 50
const MAX_RENICE_VALUE = 20

const VARZ_UPDATE_INTERVAL = 1// 1 secs
...
const DEFAULT_EVACUATION_DELAY = 30// Default time to wait (in secs) for evacuation and restart of apps.
...
Dea.prototype.run = function() {
	...
	function subscribe() {
		...
		raft.nats.subscribe('router.process_dea_status', function(msg, reply) {
			self.process_dea_status(msg, reply)
		})
		raft.nats.subscribe('healthmanager.start', function(msg, reply) {
			self.process_healthmanager_start(msg, reply)
		})
	...
	raft.common.mkdir({
		...
		var old = process.listeners('SIGINT');
		...
		self.log.write("DEA uuid " + this.uuid)
 ...
 *
 */
Dea.prototype.process_dea_locate = function(msg) {
	this.send_advertise(msg)
 ...
 *
 */
Dea.prototype.send_advertise = function() {
	var self = this
	...
	Object.keys(this.droplets).forEach(function(key) {
		self.droplets[key].forEach(function(instance) {
			if (instance['state'] == 'STARTING' || instance['state'] == 'RUNNING') {
				...
				response['usage'] = this.usage[instance['pid']]
...
Dea.prototype.process_dea_status = function() {
	...
	raft.nats.publish(reply, response.to_json)
}
/**
 *
 *
 ...
 *
 */
Dea.prototype.process_dea_stop = function(message_json) {
	var self = this
	if (this.shutting_down) {
		return
	}
	self.log.write("DEA received stop message: //{message}")
...
Dea.prototype.process_dea_start = function(message, reply) {
	...
	var tgz_file = path.join(this.staged_dir, sha1 + ".tar.gz")
	...
	if (this.secure) {
		...
		var user = grab_secure_user
		user['available'] = false
		instance['secure_user'] = user['user']
	}
	var start_operation = function() {

		self.log.write("Starting up instance " + instance['droplet'] + " on port:" + instance['port'])
