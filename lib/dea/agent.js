 ...
 *
 */
var Dea = module.exports = function() {
	...
	this.log = this.logs.logger.create('raft', 'dea', config.log_session);
...
Dea.prototype.process_dea_stop = function(message_json, reply) {
	...
	self.log.log("DEA received stop message: " + droplet_id);
...
Dea.prototype.process_dea_start = function(message, reply) {
	...
	var start_operation = function() {
		...
		spawn.on('build.port', function(info) {
			instance['port'] = info.data.port;
